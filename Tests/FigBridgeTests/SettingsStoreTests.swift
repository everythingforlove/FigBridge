...
struct SettingsStoreTests {
    @Test func defaultPromptUsesFigBridgeLocalContext() {
        let prompt = AppSettings.defaultPrompt.lowercased()

        #expect(prompt.contains("designir"))
        #expect(prompt.contains("json or yaml"))
        #expect(prompt.contains("local figma context"))
        #expect(prompt.contains("do not call figma mcp"))
        #expect(prompt.contains("do not include markdown"))
    }

    @Test func migratesLegacyFigmaMCPDefaultPromptOnLoad() throws {
        let sandbox = try TestSandbox()
        defer { sandbox.cleanup() }
        let store = SettingsStore(fileURL: sandbox.root.appendingPathComponent("settings.json"))
        var settings = AppSettings.defaultValue
        settings.promptTemplate = AppSettings.legacyFigmaMCPDefaultPrompt

        try store.save(settings)
        let loaded = try store.load()

        #expect(loaded.promptTemplate == AppSettings.defaultPrompt)
    }

    @Test func migratesOriginalYAMLFigmaMCPDefaultPromptOnLoad() throws {
        let sandbox = try TestSandbox()
        defer { sandbox.cleanup() }
        let store = SettingsStore(fileURL: sandbox.root.appendingPathComponent("settings.json"))
        var settings = AppSettings.defaultValue
        settings.promptTemplate = AppSettings.legacyYAMLFigmaMCPDefaultPrompt
